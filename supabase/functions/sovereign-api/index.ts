...
serve(async (req) => {
  ...
  try {
    ...
    if (action === 'referral:get') {
      ...
      let { data: profile, error: pErr } = await sb.from('user_profiles').select('referral_code, referral_credits').eq('id', user.id).single();
      if (pErr || !profile) {
        // Upsert profile to trigger code generation
        const { data: up, error: upErr } = await sb.from('user_profiles')
          .upsert({ id: user.id, email: user.email, full_name: user.user_metadata?.full_name || null })
          .select('referral_code, referral_credits').single();
        if (upErr) return json({ error: upErr.message }, 500);
        profile = up;
      }
      // Count referrals by status
      const { data: refs } = await sb.from('referrals').select('status').eq('referrer_id', user.id);
    ...
    if (action === 'referral:track') {
      ...
      if (referrerProfile.id === user.id) return json({ error: 'Cannot refer yourself' }, 400);
      // Check not already tracked
      const { data: existing } = await sb.from('referrals').select('id').eq('referred_user_id', user.id).single();
      ...
      await sb.from('user_profiles').upsert({ id: user.id, referred_by: code });
      return json({ ok: true });
    }
